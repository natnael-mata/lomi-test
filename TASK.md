...
**167 of 189 tasks done.** Generated by `node scripts/task-progress.mjs` —
do not edit by hand.
...
- [ ] **T-160** Admin dashboard: signups, trials, activations, awaiting verification, expired.
  ...
  > blocked: only "signups" is countable today. Trials, activations and expired are `Subscription`
  > figures and "awaiting verification" is a Fayda one — both land with Phase 8. Building the
  > dashboard now would mean four panels reading zero, which is worse than an empty screen: a
  > zero is a measurement, and nobody has measured anything.
- [ ] **T-161** Revenue split by Chapa vs bank, footing to a total.
      **Test:** Chapa + bank equals the total, asserted in a test.
  > blocked: there is no `Payment` model yet. Phase 8 owns it.
...
- [ ] **T-163** User search by phone, display name or transaction reference.
  ...
  > blocked on its stated test only. Phone and display name are searchable now; `txRef` needs
  > `Payment` from Phase 8, and the test names it specifically. Deferred whole rather than
  > shipped in halves — a search box that silently ignores one of the three things it says it
  > accepts is worse than one that is not there yet.
