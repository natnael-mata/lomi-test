...
**108 of 187 tasks done.** Generated by `node scripts/task-progress.mjs` —
do not edit by hand.
...
| Phase 6 — Exam mode                              | 14/15 | 93%  | 1 left           |
...
- [x] **T-125** One question at a time with Back/Next and a jump-to grid.
      **Test:** Navigating away and back preserves the selected option. > Nothing is held in the client: every choice is written as it is made, so > "navigate away and back" is a re-read, not a restore. The e2e test walks > the whole paper past two answered questions and comes back to both.
- [x] **T-126** Flagging a question persists across navigation and reload.
      **Test:** Flag Q13, reload, Q13 is still flagged. > Reload is a fresh start request, which rejoins the sitting in flight. > The test also pins the failure mode the obvious implementation has: > flagging sends no `chosenLabel`, so a handler that writes the whole row > erases the answer. Verified — it fails against that version.
- [x] **T-127** The jump grid distinguishes answered / flagged / current by shape or icon,
      not colour alone.
      **Test:** In greyscale, all three states remain distinguishable. > Verified in a greyscale render. The three backgrounds collapse to > near-identical greys (luminance 237 / 243 / 255) and so do the text > colours — colour carries nothing, which is the point. What remains is > ● answered, ○ unanswered, ⚑ flagged, and a 2px border for current.
- [x] **T-128** Timer changes state at 20% and 5% remaining, without blinking.
      **Test:** No `animation` on the timer element at any threshold. > Browser-verified at all three states: `animationName: none`, no child > animations, `transitionDuration: 0s`. Thresholds are fractions, not > fixed minutes — 20% of a three-hour mock is 36 minutes and 20% of a > ten-minute drill is two. Tabular figures keep the width from jittering, > which reads as blinking even when nothing animates.
- [x] **T-129** On submit, the full review unlocks: every question with its explanation.
      **Test:** All 60 answer payloads are retrievable after close, none before. > Both halves were already enforced server-side; this added the screen. > Everything unlocks at once — no accordions, no drip-feed. The review > paginates at 10 for first-paint speed on a phone, which is a rendering > decision and not a gate: every explanation is already in the response. > It also re-reads the closed sitting rather than trusting the submit > response, because a sitting closes on the deadline too, and both paths > should land on the same screen.
- [x] **T-130** Post-exam summary shows score, per-topic breakdown and the weakest topic by
      blueprint weight.
      **Test:** Weakest topic is chosen by weight × miss rate, not raw misses. > Deliberately **not** `practice/summary.ts`. Practice ranks by lowest > score ("what am I worst at"); a mock ranks by expected marks lost > ("where would another hour buy me the most"). The two disagree, and the > unit test pins a case where raw misses and the real rule point at > different topics. > > An unweighted topic reports `null`, not zero, and can never win the > ranking — unknown is not the same as costing nothing. A paper where > _nothing_ is weighted falls back to raw misses, which is what the bank > looks like until T-134 lands. > > Note: weight × miss rate is only a distinct rule when the weight comes > from the syllabus. Using the paper's own composition collapses it to > `misses ÷ paper size` — literally raw misses over a constant — so this > genuinely depends on `Topic.weightPct`, i.e. on T-134.
